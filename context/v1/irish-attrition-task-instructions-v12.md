# Task Instructions & Screen Design (v12)
For platform build — lexical task portion

## Package summary — what's being sent to the collaborator

1. **Questionnaire** (v35, .md + .docx) — full instrument, consent, predictors, background variables
2. **Word list** (v13, .xlsx, "FINAL 60 – Selected" sheet) — the 60 test items
3. **Codebook** (v24) — variable definitions, scoring protocol, analysis plan
...
1. **Consent screen** (questionnaire v35, Part 0)
...
- Timer: 20 seconds. Do NOT show a numeric countdown — use a subtle, non-alarming visual cue instead (e.g. a slowly depleting progress bar with no numbers). Rationale: a visible ticking countdown can itself suppress recall through performance anxiety, which is a real risk for this population given the emotional weight many participants attach to re-engaging with a language they feel they've lost (cf. the reconnection-motivation items in the questionnaire). A fully invisible timer was also rejected, since items cutting off with zero warning risks feeling abrupt or unfair. Auto-advances on timeout regardless.
...
- Timer: 20 seconds, same subtle-progress-indicator approach as recall (no numeric countdown) — see rationale above
...
## Screen 6 — Progress indicator

- Simple progress indicator throughout ("Item 23 of 60")
- No break screen — deliberately removed. A single continuous 60-item block was judged preferable to interrupting participants' momentum partway through.
...
Keep `response_text` as raw typed input — final scoring (fada/edit-distance tolerance, manual review) is applied afterward per the codebook (Section E). **Explicit clarification: typing WITH correct fadas is never a problem and is always accepted** — the scoring protocol strips fadas from both the target and the response before comparing, so a participant who correctly includes fadas scores exactly the same as one who omits them. Nothing needs to be built differently either way; the platform just needs to record whatever was typed, fadas or not.
