...
#'
#' @param cao_lookup Optional data frame with columns `leaving_cert_paper`,
#'   `leaving_cert_grade`, `points` -- a grade-to-points conversion table
#'   (year-independent; see analysis/resources/cao_lookup.csv). If omitted,
#'   `proficiency_score` is `NA` for every row (with a warning) rather than
#'   guessed.
compute_predictors <- function(questionnaire_wide, cao_lookup = NULL) {
  ...
  if (is.null(cao_lookup)) {
    warning(
      ...
      "is NA for all rows. Supply a grade-to-points table (columns: ",
      "leaving_cert_paper, leaving_cert_grade, points) once available."
    )
    qw$proficiency_score <- NA_real_
  } else {
    # Per the researcher's current CAO lookup, the score depends only on
    # grade + paper level -- no exam year at all (a fixed points scale, not
    # a year-by-year historical one), and the questionnaire's Q14 grade
    # (A/B/C/D/E/F) is used as-is with no sub-grade to resolve.
    #
    # NOTE: the questionnaire's `leaving_cert_paper` values are "Higher
    # level"/"Ordinary level"/"Foundation level"; cao_lookup uses the bare
    # "Higher"/"Ordinary"/"Foundation". Normalize both sides so the join
